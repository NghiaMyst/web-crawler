...
- **Global scope**: `D:/project/mcp/web-crawler/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/project/mcp/web-crawler/.claude/notes/`)
...
2. Glob `D:/project/mcp/web-crawler/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/project/mcp/web-crawler/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/project/mcp/web-crawler/.claude/notes/` — works in any directory
